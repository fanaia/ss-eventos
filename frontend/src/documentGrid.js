...
/** Formatação exclusivamente visual; o valor persistido não é alterado. */
...
/** Aplica o renderer de CPF/CNPJ na listagem de Clientes/Fornecedores. */
export function aplicarMascaraDocumentoNoGrid(manifest) {
  return {
    ...manifest,
    collections: manifest.collections?.map((collection) => {
      if (collection.model !== "ClienteFornecedor") return collection;
